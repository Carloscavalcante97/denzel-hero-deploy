...
const items = [
  { href: "#galeria-container", icon: "/menuEventos.svg", label: "Eventos" },
  { href: "#enviar-file-container", icon: "/menuCompartilhe.svg", label: "Compartilhe" },
  { href: "#quem-somos-carousel", icon: "/menuDenzel.svg", label: "A DENZEL" },
  { href: "#contato-container", icon: "/menuContato.svg", label: "Contato" },
];
          ...
          {items.map(({ href, icon, label }) => (
            <li key={href} className="border-b border-white/10">
              <a
                href={href}
                onClick={onClose}
                className="block p-[1px]" // espessura da borda gradiente
              >
                <div
                  className="
